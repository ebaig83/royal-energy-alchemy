...
exports.handler=async event=>{
 try{
  ...
  if(event.queryStringParameters?.diagnostics==='true'){const probed=await providerHealth(process.env);data.diagnostics.services=data.diagnostics.services.map(s=>{const p=probed.find(p=>p.name===s.name);return p?{...s,...p}:s;});}data.authenticated=true;data.user=auth.user;try{data.security=require('./lib/practitioner-credential').metadata(await require('./lib/practitioner-credential').read(require('./lib/supabase').getClient()));}catch{data.security=null;}
