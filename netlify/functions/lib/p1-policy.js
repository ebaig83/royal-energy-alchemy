...
function diagnostics(d,env={},now=new Date(),heartbeats=[]){
 ...
 return {checked_at:now.toISOString(),counts:{failedCalendar:calendar.length,pendingWorkflows:stuck.length,stuckWorkflows:stuck.length?null:0,failedCommunications:failures.length},services:[
