...
async function recoveryRequestAttempt(sb,event,emailHash){const ip=String(event.headers?.['x-nf-client-connection-ip']||event.headers?.['client-ip']||event.headers?.['x-forwarded-for']||'unknown').split(',')[0].trim();const bucket=crypto.createHash('sha256').update('recovery-request:'+ip+':'+String(emailHash||'')).digest('hex');const {data,error}=await sb.rpc('practitioner_recovery_request_attempt',{p_bucket:bucket});if(error)throw Error('Security verification unavailable.');return data===true;}
...
module.exports={hash,verify,validNew,read,attempt,recoveryAttempt,recoveryRequestAttempt,metadata};
