...
async function processNotices(sb,send=sendWithPreferences){
 ...
 for(const row of rows||[]){try{
  ...
  else if(!['manage_appointment','dashboard','expire-payment-holds','expire-website-bookings'].includes(row.source))finish='unknown_legacy';
  ...
  if(!finish){const {data:c,error:clientError}=await sb.from('clients').select('email,email_consent').eq('id',s.client_id).maybeSingle();if(clientError)throw Error();
   ...
   else {const origin=row.source==='manage_appointment'?'client':row.source==='dashboard'?'practitioner':'system';const response=await send(sb,{templateName:row.action==='reschedule'?'appointment_rescheduled':'appointment_cancelled',recipientEmail:c.email,clientId:s.client_id,sessionId:s.id,idempotencyKey:'appointment-change:'+row.id,metadata:{session_id:s.id,appointment_change_id:row.id,correlation_id:row.correlation_id||row.id,trigger:origin+'_'+row.action},variables:{client_name:s.client_name,service:s.service,old_date:row.old_date,old_time:row.old_time,new_date:row.new_date,new_time:row.new_time,session_date:s.session_date,session_time:s.session_time,timezone:'ET',duration_minutes:s.duration_minutes,duration:s.duration_minutes||60,location_type:s.location_type,google_meet_url:/^https:\/\/meet\.google\.com\/[a-z-]+$/.test(s.google_meet_url||'')?s.google_meet_url:'',manage_url:appointmentManageUrl(s.id),contact_email:process.env.ADMIN_EMAIL||'droyal168@gmail.com',refund_summary:'Cancellation does not automatically issue a refund.',policy_line_1:'Contact Daron with any questions.',policy_line_2:'',policy_line_3:'',policy_line_4:''}});
