...
export function appointment(s){if(['online','website','website_booking','website booking','website_form','booking'].includes(String(s.source||'').trim().toLowerCase())&&s.payment_status!=='paid'&&!['cancelled','expired','completed','no_show'].includes(String(s.status||'').toLowerCase()))return ['Payment required','amber'];return ({confirmed:['Confirmed','blue'],pending:['Pending confirmation','amber'],ready:['Confirmed','blue'],completed:['Completed','green'],cancelled:['Cancelled','muted'],expired:['Expired — payment not received','muted'],no_show:['Missed appointment','amber']})[s.status]||['Review appointment','muted'];}
