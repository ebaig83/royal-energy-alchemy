...
const valid = { client_name: 'Ada Example', client_email: ' ADA@EXAMPLE.COM ', client_phone: '(814) 555-0199' };
assert.deepEqual(_test.validatePublicBookingFields(valid), {
  name: 'Ada Example', email: 'ada@example.com', phone: '8145550199',
});
for (const body of [
...
assert.equal(_test.isBlockedClient({ email: 'blocked@example.com', phone: '(814) 555-0199' }, 'blocked@example.com', '8145550199'), true);
assert.equal(_test.isBlockedClient({ email: 'other@example.com', phone: '5555555555' }, 'blocked@example.com', '8145550199'), false);
